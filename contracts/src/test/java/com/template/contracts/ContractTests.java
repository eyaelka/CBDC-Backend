...
public class ContractTests {
    ...
    public void issuerAndRecipientCannotHaveSameEmail() {
        /*CentralBankStoreState state = new CentralBankStoreState("Hello-World",alice.getParty(),bob.getParty());
        ledger(ledgerServices, l -> {
            l.transaction(tx -> {
                tx.input(CentralBankStoreContract.ID, state);
                tx.output(CentralBankStoreContract.ID, state);
                tx.command(alice.getPublicKey(), new CentralBankStoreContract.Commands.Send());
