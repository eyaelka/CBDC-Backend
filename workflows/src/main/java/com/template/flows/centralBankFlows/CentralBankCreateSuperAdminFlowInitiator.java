...
@InitiatingFlow
@StartableByRPC
public class CentralBankCreateSuperAdminFlowInitiator extends FlowLogic<AccountIdAndPassword> {
    private final CentralBankAccountInfo centralBankAccountInfo;
    private final ProgressTracker progressTracker = new ProgressTracker();

    public CentralBankCreateSuperAdminFlowInitiator(CentralBankAccountInfo centralBankAccountInfo) {
    ...
    private AccountIdAndPassword generateAccountIdAndPasswordRandomly(){
        ...
        //super admin
        AccountIdAndPassword accountIdAndPassword = new AccountIdAndPasswordGenerator("ad").generateAccountIdAdnPassword();

        int index;
        while ((index = containsAccountIdOrPassword(accountIdAndPassword)) ==1){
            accountIdAndPassword = new AccountIdAndPasswordGenerator("ad").generateAccountIdAdnPassword();
        }
        if (index == -1)
            return null;
        return accountIdAndPassword;
